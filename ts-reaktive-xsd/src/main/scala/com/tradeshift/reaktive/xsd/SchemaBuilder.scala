...
class SchemaBuilder extends GraphStageWithMaterializedValue[SinkShape[SchemaEntry],Future[Schema]] {
  import SchemaItem._
  import SchemaBuilder._
  ...
  override def createLogicAndMaterializedValue(attr: Attributes) = {
    ...
    def safely[T](block: =>T) = try { block } catch { case x:Throwable => mat.failure(x); throw x }
    
    (new GraphStageLogic(shape) {
      /** a cache of Namespace instances */ 
      val namespace = Map.empty[String,Namespace].withDefault(s => ns(s))
      val types = new ResolveMap[QName,RootType](XsdBuiltinType.all.toSeq: _*)
      var elements = new ResolveMap[QName,RootElement]
