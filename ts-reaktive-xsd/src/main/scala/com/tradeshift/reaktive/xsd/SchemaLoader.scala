...
object SchemaLoader {
  /**
    ...
    * @param mainNamespace Main first namespace to load (import and include tags are followed from there,
    *                      recursively.)
    ...
    */
  def apply(mainNamespace: String, normalize: Import => Import, load: Import => Source[XMLEvent,_])(implicit m: Materializer): Future[Schema] = {

    RunnableGraph.fromGraph(GraphDSL.create(new SchemaBuilder()) { implicit b => schemaBuilder =>
      import GraphDSL.Implicits._

      val packageNextImports: Flow[AnyRef,Set[Import],NotUsed] = Flow[AnyRef].statefulMapConcat { () =>
        var nextImports = Set.empty[Import]
        elem => elem match {
          case SchemaEntry(_, i@Import(_,_)) =>
      ...
      val kick = Source.single(Set(normalize(Import(mainNamespace, None))))
